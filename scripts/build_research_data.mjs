...
async function main() {
  ...
  try {
    const laoshengProfiles = await readCsvObjects(path.join(researchDir, 'laosheng_school_profiles.2026-04-22.csv'));
    counts.laoshengRows = laoshengProfiles.length;
    for (const row of laoshengProfiles) {
      if (!row.moeCode || !officialByCode.has(row.moeCode)) continue;
      const profile = ensureProfile(profiles, row.moeCode);
      if (!profile.website && row.website?.trim()) {
        profile.website = row.website.trim();
        counts.laoshengWebsiteFillSchools += 1;
      }
      if (!profile.admissionWebsite && row.admissionWebsite?.trim()) {
        profile.admissionWebsite = row.admissionWebsite.trim();
        counts.laoshengAdmissionFillSchools += 1;
      }
    }
  } catch {
    counts.laoshengRows = 0;
  }
  ...
  counts.mergedAdmissionSchools = Object.values(schoolResearchProfilesByMoeCode).filter((profile) => Boolean(profile.admissionWebsite)).length;
  ...
  console.log(`merged website profiles: ${counts.mergedWebsiteSchools}`);
  console.log(`laosheng website fills: ${counts.laoshengWebsiteFillSchools}`);
  console.log(`laosheng admission fills: ${counts.laoshengAdmissionFillSchools}`);
  console.log(`merged admission profiles: ${counts.mergedAdmissionSchools}`);
