...
export const DIMENSIONS: Record<DimensionId, DimensionMeta> = {
  ...
  A6: {
    ...
    coverage: 'mixed',
    notes: '藝術/體育/軍警/航海的校名提示不參與整校排除，四個硬篩選值保留待逐省逐年官方依據；comprehensive_eval（該省只走綜合評價、須另行報名+校測）只認各校招生章程，逐省逐年記錄在 data/research/admission_channels.*.csv。'
