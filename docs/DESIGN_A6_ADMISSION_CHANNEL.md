...
### 7.6 上線前實測揪出的第三個錯：省份繁簡不一致

第二輪數據寫完、測試全綠、線上也發過一次之後，用瀏覽器逐省點下拉選單複驗時發現：
上海會多排除一所（正確），江蘇卻沒有。

根因：全站省份字串是繁體（`江蘇`、`廣東`、`山東`，來自教育部主表），
而招生章程原文是簡體，我照抄進 CSV。於是 `'江苏' !== '江蘇'`，逐省匹配全部落空，
落到「收錄了但沒覆蓋該省」分支、回到 regular 先驗——**界面一切正常，規則整條沒生效**。
只有那些繁簡同形的省份（北京、上海、天津、浙江、福建…）碰巧是對的。

更值得記的是：**測試沒攔住，因為測試用了和數據同一套錯誤寫法。**
我在測試裡手寫 `'江苏'` 傳進引擎，數據裡也是 `'江苏'`，兩邊自洽，全綠。

兩道修補：

1. `build_admission_channels.mjs` 用教育部主表的省份集合做白名單，
   附一張簡→繁別名表做歸一，歸一不了的 `exit 1`（已用 `江甦` 做過反向驗證）。
2. 新增測試直接拿 `candidateProvinceOptions`（考生地區下拉選單真正會送進來的值）
   去校驗每一條記錄的省份，並斷言「江蘇比北京多排除一所」。

教訓寫進手冊 §3.1a：測資料的斷言不能和資料共用同一套約定，要從界面實際會產生的值取。
