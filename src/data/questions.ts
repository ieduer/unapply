...
const sectionA: Question[] = [
  ...
  },
  {
    ...
    dataStatus: 'mixed',
    title: '學費能接受到哪一檔？',
    subtitle: '教育部名單能權威分出公辦、民辦、中外合作三類：公辦約 5-6 千，民辦本科多在 1.5-3 萬，中外合作辦學機構 6-25 萬。逐校精確金額仍要看當年招生章程，這裡不硬填。',
    type: 'single',
    skippable: true,
    options: [
      { key: 'public', label: '只要公辦（每年約 5-6 千）',
        excludes: [{ dim: 'A4', values: ['1-3萬', '3-8萬', '8萬+', '民辦待核價', '中外合作待核價'] }] },
      { key: 'private_ok', label: '公辦或民辦都行，但不接受中外合作的高學費',
        excludes: [{ dim: 'A4', values: ['3-8萬', '8萬+', '中外合作待核價'] }] },
      { key: 'any', label: '不限' },
    ],
  },
  {
  ...
  },
  {
    ...
    dataStatus: 'mixed',
    title: '要不要另外報名、另外考一場？',
    subtitle: '除了填志願，有些學校還得單獨報名、校考、體測、政審，或參加綜合評價校測。判定按你的考生地區來——同一所學校對不同省份的口徑可以完全不同：上海科技大學只有江蘇考生必須拿到校園開放日成績，其餘省份裸分就能填。',
    type: 'single',
    skippable: true,
    options: [
      { key: 'regular_only', label: '只看填志願就能錄取的（不另報名、不校測）',
        excludes: [{ dim: 'A6', values: ['comprehensive_eval', 'art_exam', 'sports_test', 'military_police', 'navigation_flight'] }] },
      { key: 'no_comprehensive', label: '排掉要綜合評價校測的',
